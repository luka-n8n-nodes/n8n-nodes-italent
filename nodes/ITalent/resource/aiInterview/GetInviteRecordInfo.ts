...
const GetInviteRecordInfoOperate: ResourceOperations = {
	name: '获取AI面试应聘者信息',
	value: 'getInviteRecordInfo',
	action: '获取AI面试应聘者信息',
	options: [
		{
		...
		{
			displayName: 'Ext Columns',
			name: 'extColumns',
			type: 'string',
			default: '',
			description: 'Array of extra columns. Can be a JSON string or an expression that returns an array.',
			placeholder: '["name", "height", "weight", "Email"] or {{ $json.extColumns }}',
		},
		{
			displayName: 'Options',
			name: 'options',
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject | IDataObject[]> {
		...
		const extColumnsInput = this.getNodeParameter('extColumns', index, '') as string | string[];
		const options = this.getNodeParameter('options', index, {}) as RequestOptions;
		
		let inviteIds: string[] = [];
		let extColumns: string[] = [];
		...
		// 处理 extColumns 输入
		if (Array.isArray(extColumnsInput)) {
			extColumns = extColumnsInput;
		} else if (typeof extColumnsInput === 'string') {
			const trimmed = extColumnsInput.trim();
			if (trimmed !== '') {
				try {
					const parsed = JSON.parse(trimmed);
					if (Array.isArray(parsed)) {
					...
					throw new Error('Invalid input for extColumns. Expected a JSON array string or an expression that returns an array.');
				}
			}
		} else if (extColumnsInput) {
			throw new Error('Invalid input type for extColumns. Expected a string or array.');
		}
		...
		if (inviteIds.length === 0) {
			throw new Error('邀请ID集合不能为空，请至少提供一个邀请ID');
		}
		...
		if (extColumns.length > 0) {
			body.extColumns = extColumns;
		}

		// 构建请求选项
		const requestOptions: any = {
			method: 'POST',
			url: '/AIInterview/api/v1/AIInterviewInvite/GetInviteRecordInfo',
			body,
			json: true,
		};
...
export default GetInviteRecordInfoOperate;
