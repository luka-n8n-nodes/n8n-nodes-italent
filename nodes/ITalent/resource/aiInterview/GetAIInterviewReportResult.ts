...
const GetAIInterviewReportResultOperate: ResourceOperations = {
	name: '获取作答结果',
	value: 'getAIInterviewReportResult',
	action: '获取作答结果',
	options: [
		{
			...
			type: 'string',
			default: '',
			description: 'Array of invite IDs (max 10). Can be a JSON string or an expression that returns an array.',
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject | IDataObject[]> {
		...
		if (inviteIds.length > 10) {
			throw new Error(`邀请ID集合不能超过10个，您提供了${inviteIds.length}个，请减少ID数量`);
		}
		...
		const requestOptions: any = {
			method: 'POST',
			url: '/AIInterview/api/v1/ReportResult/GetAIInterviewReportResult',
			body: {
				inviteIds,
			},
			json: true,
...
export default GetAIInterviewReportResultOperate;
