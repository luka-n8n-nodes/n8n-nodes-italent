...
const DEFAULT_DATA_PATH = 'questionItems';
const DEFAULT_NEXT_BATCH_ID_PATH = 'nextBatchId';

const GetWorkIntentionQuestionsOperate: ResourceOperations = {
	name: '获取工作意向题目详情',
	value: 'getWorkIntentionQuestions',
	action: '获取工作意向题目详情',
	options: [
		{
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject | IDataObject[]> {
		...
		const fetchPage = async (batchId: string = ""): Promise<any> => {
			...
			return await RequestUtils.request.call(this, {
				method: 'POST',
				url: '/AIInterview/api/v1/AIInterviewQuestion/GetWorkIntentionQuestions',
				body,
				json: true,
			});
		};
		...
		if (!returnAll) {
			...
			// 如果存在 questionItems 字段，返回 questionItems 数组，否则返回原始响应
...
export default GetWorkIntentionQuestionsOperate;
