...
const GetSysMessageTemplateListOperate: ResourceOperations = {
	name: '获取所有启用的AI面试通知模板数据',
	value: 'getSysMessageTemplateList',
	action: '获取所有启用的AI面试通知模板数据',
	options: [
		{
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject | IDataObject[]> {
		...
		const fetchPage = async (batchId: string = ""): Promise<any> => {
			...
			return await RequestUtils.request.call(this, {
				method: 'POST',
				url: '/AIInterview/api/v1/SysMessageTemplate/GetSysMessageTemplateList',
				body,
				json: true,
			});
		};
...
export default GetSysMessageTemplateListOperate;
