...
const GetInviteTestUrlOperate: ResourceOperations = {
	name: '获取邀请链接',
	value: 'getInviteTestUrl',
	action: '获取邀请链接',
	options: [
		{
			...
			type: 'string',
			default: '',
			description: 'Array of invite IDs (max 20). Can be a JSON string or an expression that returns an array.',
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject | IDataObject[]> {
		...
		if (inviteIds.length > 20) {
			throw new Error(`邀请ID集合不能超过20个，您提供了${inviteIds.length}个，请减少ID数量`);
		}
		...
		const requestOptions: any = {
			method: 'POST',
			url: '/AIInterview/api/v1/AIInterviewInvite/GetInviteTestUrl',
			body: {
				inviteIds,
			},
			json: true,
...
export default GetInviteTestUrlOperate;
